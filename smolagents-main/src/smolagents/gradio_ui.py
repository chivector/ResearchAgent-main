...
def get_step_footnote_content(
    ...
    if step_log.token_usage is not None:
        step_footnote += (f" | Input tokens: {step_log.token_usage.input_tokens:,}"
                          f" | Output tokens: {step_log.token_usage.output_tokens:,}")
        # 累计统计仅在提供计数器时输出
        if token_counters is not None:
            token_counters["input"] = token_counters.get("input", 0) + step_log.token_usage.input_tokens
            token_counters["output"] = token_counters.get("output", 0) + step_log.token_usage.output_tokens
            cumulative_line = (f"Input tokens (cumulative): {token_counters['input']:,}"
                               f" | Output tokens (cumulative): {token_counters['output']:,}")
    step_footnote += f" | Duration: {round(float(step_log.timing.duration), 2)}s" if step_log.timing.duration else ""
    step_footnote_content = f"""<span style="color: #bbbbc2; font-size: 12px;">{step_footnote}</span> """
    if cumulative_line:
        step_footnote_content += (f"""<br/><span style="color: #bbbbc2; font-size: 12px;">{cumulative_line}</span> """)
...
def _process_action_step(
    ...
    if getattr(step_log, "error", None):
        yield gr.ChatMessage(role=MessageRole.ASSISTANT,
    ...
    yield gr.ChatMessage(
        role=MessageRole.ASSISTANT,
        content=get_step_footnote_content(step_log, step_number, token_counters),
...
def _process_planning_step(
    ...
    yield gr.ChatMessage(
        role=MessageRole.ASSISTANT,
        content=get_step_footnote_content(step_log, "Planning step", token_counters),
        metadata={"status": "done"},
    )
    yield gr.ChatMessage(role=MessageRole.ASSISTANT, content="-----", metadata={"status": "done"})
...
def pull_messages_from_step(
    ...
    if isinstance(step_log, ActionStep):
        yield from _process_action_step(step_log, skip_model_outputs, token_counters)
    elif isinstance(step_log, PlanningStep):
        yield from _process_planning_step(step_log, skip_model_outputs, token_counters)
...
def stream_to_gradio(
    ...
    # 用于 Gradio 界面累计 token 统计
    token_counters = {"input": 0, "output": 0}
