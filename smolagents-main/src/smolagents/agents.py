...
class MultiStepAgent(ABC):
    ...
    def _setup_step_callbacks(self, step_callbacks):
        ...
        # Register monitor update_metrics for action + planning steps
    ...
    def run(
        ...
        self.monitor.mark_global_start()
        if additional_args:
...
{str(additional_args)}."""
    ...
    def _run_stream(
        ...
        while not returned_final_answer and self.step_number <= max_steps:
            ...
            step_title = f"{self.name} | Step {self.step_number}" if self.name else f"Step {self.step_number}"
            self.logger.log_rule(step_title, level=LogLevel.INFO)
            try:
