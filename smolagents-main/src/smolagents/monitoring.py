...
_GLOBAL_TOTAL_INPUT = 0
_GLOBAL_TOTAL_OUTPUT = 0
_GLOBAL_STEP_COUNT = 0


def _ensure_global_start() -> float:
...
class Monitor:
    ...
    def mark_global_start(self) -> None:
        with _GLOBAL_STATS_LOCK:
            _ensure_global_start()
    ...
    def update_metrics(self, step_log):
        ...
        """
        global _GLOBAL_TOTAL_INPUT, _GLOBAL_TOTAL_OUTPUT, _GLOBAL_STEP_COUNT
        step_duration = step_log.timing.duration
        self.step_durations.append(step_duration)

        with _GLOBAL_STATS_LOCK:
            _GLOBAL_STEP_COUNT += 1
            global_step_number = _GLOBAL_STEP_COUNT

        console_outputs = f"[Step {global_step_number}: Duration {step_duration:.2f} seconds"

        if step_log.token_usage is not None:
            step_input_tokens = step_log.token_usage.input_tokens
            step_output_tokens = step_log.token_usage.output_tokens
            self.total_input_token_count += step_input_tokens
            self.total_output_token_count += step_output_tokens
            with _GLOBAL_STATS_LOCK:
                global_start_time = _ensure_global_start()
                _GLOBAL_TOTAL_INPUT += step_input_tokens
                _GLOBAL_TOTAL_OUTPUT += step_output_tokens
                global_input_tokens = _GLOBAL_TOTAL_INPUT
                global_output_tokens = _GLOBAL_TOTAL_OUTPUT
                global_elapsed = time.time() - global_start_time
            console_outputs += (
                f"| Input tokens: {step_input_tokens:,} | Output tokens: {step_output_tokens:,}"
            )
        console_outputs += "]"
        self.logger.log(Text(console_outputs, style="dim"), level=1)
        if step_log.token_usage is not None:
            cumulative_line = (
                f"[Cumulative | Global elapsed: {global_elapsed:.2f} seconds | "
                f"Input tokens: {global_input_tokens:,} | Output tokens: {global_output_tokens:,}]"
            )
            self.logger.log(Text(cumulative_line, style="dim"), level=1)
